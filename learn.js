(() => {
  const categoryTree = window.LEARN_CATEGORY_TREE || [];
  ...
  function genderFrom(article, plural) {
    const cleanArticle = normalize(article).replace("'", "’");
  ...
  function buildLearnEntries() {
    ...
    if (typeof buildVocabItems === "function") {
      buildVocabItems().forEach((item) => {
        add({
          ...
          article: articleOf(item.it),
          gender: articleOf(item.it) ? genderFrom(articleOf(item.it), "") : ""
        });
      });
    }

    return entries.sort((a, b) => a.it.localeCompare(b.it, "it"));
  }
  ...
  function inferAdjectiveForms(forms) {
    ...
    return unique.map((form, index) => [labels[index], form]);
  }
  ...
  function classifyEntry(entry) {
    ...
    if (hay.includes("pronoun")) return folder("Basic vocabulary", "Greetings and introductions");
    if (hay.includes("question")) return folder("Basic vocabulary", "Greetings and introductions");
    if (hay.includes("preposition")) return folder("Basic vocabulary", "Describing");
    if (hay.includes("connector") || hay.includes("adverb")) return folder("Basic vocabulary", "Describing");

    return folder("Basic vocabulary", "Describing");
  }
  ...
  function renderLearn(view = currentView) {
    ...
    document.querySelectorAll("[data-learn-view]").forEach((button) => {
      button.addEventListener("click", () => renderLearn(button.dataset.learnView));
    });
    bindLearnBody();
  }
  ...
  function renderVocabularyLibrary() {
    ...
    const activeEntries = selectedFolder ? entries.filter((entry) => `${entry.major}|${entry.minor}` === selectedFolder) : entries.slice(0, 18);
    const selectedEntry = activeEntries.find((entry) => entry.id === selectedEntryId) || activeEntries[0] || entries[0];

    return `
      <div class="learn-library-layout">
        <aside class="learn-folder-panel">
          ...
          <div class="learn-folder-scroll">
            ${grouped.map((section) => renderFolderSection(section)).join("")}
          </div>
        </aside>
        <section class="learn-word-panel">
          <div class="learn-word-list">
            ${(activeEntries.length ? activeEntries : entries).slice(0, 80).map((entry) => `<button class="learn-word-row ${selectedEntry?.id === entry.id ? "selected" : ""}" type="button" data-entry-id="${entry.id}"><span>${escapeHtml(entry.it)}</span><small>${escapeHtml(entry.en)}</small></button>`).join("")}
          </div>
  ...
  function nounRule(entry) {
    ...
    if (entry.it.endsWith("o")) return "Masculine nouns ending in -o usually become -i in the plural.";
    if (entry.it.endsWith("a")) return "Feminine nouns ending in -a usually become -e in the plural, with spelling changes when needed.";
  ...
  function renderPlainWordCard(entry) {
    return `<article class="learn-detail-card">
      ...
      <h3>${escapeHtml(entry.it)}</h3>
      <p class="learn-translation">${escapeHtml(entry.en)}</p>
      <p class="learn-note">Learn this form as a fixed exam word or phrase.</p>
    </article>`;
  }
  ...
  function verbEntries() {
    const entries = [];
    if (typeof VERBS !== "undefined") entries.push(...VERBS);
    return entries;
  }
  ...
  function verbFromEntry(entry) {
    if (typeof VERBS !== "undefined") {
      return VERBS.find((verb) => verb.inf === entry.it);
    }
    return null;
  }
  ...
  function bindLearnBody() {
    document.querySelectorAll(".learn-overview-card[data-learn-view]").forEach((button) => {
      button.addEventListener("click", () => renderLearn(button.dataset.learnView));
    });
    document.querySelector("#learnSearch")?.addEventListener("input", (event) => {
